...
def _forward_on_query(
    ...
    if (conf is not None) and (points_3d is not None):
        ...
        if valid_mask.sum() > 512:
            # 将 valid_mask 转换为 CPU numpy 数组（因为 pred_color 已经是 numpy 数组）
            ...
            pred_color = pred_color[valid_mask_np]  # 使用 numpy 数组索引
    else:
        pred_conf = None
        pred_point_3d = None
    ...
    # 分离梯度计算图后再转换为 numpy（推理模式下需要）
    pred_track = pred_track.squeeze(0).float().detach().cpu().numpy()
    pred_vis = pred_vis.squeeze(0).float().detach().cpu().numpy()
    
    # 将 pred_conf 和 pred_point_3d 转换为 numpy（如果它们不是 None）
    if pred_conf is not None:
        pred_conf = pred_conf.detach().cpu().numpy() if isinstance(pred_conf, torch.Tensor) else pred_conf
    if pred_point_3d is not None:
        pred_point_3d = pred_point_3d.detach().cpu().numpy() if isinstance(pred_point_3d, torch.Tensor) else pred_point_3d
